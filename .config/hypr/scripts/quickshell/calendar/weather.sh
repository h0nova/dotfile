...
current_file="${cache_dir}/current_weather.json"
...
get_data() {
    ...
    # ---------------------------------------------------------
    # Fetch current weather (real-time, not 3h forecast)
    current_url="https://api.openweathermap.org/data/2.5/weather?id=${ID}&appid=${KEY}&units=${UNIT}"
    raw_current=$(curl -sf "$current_url")
    if [ -n "$raw_current" ] && [ "$(echo "$raw_current" | jq -r '.cod')" = "200" ]; then
        c_temp=$(printf "%.1f" "$(echo "$raw_current" | jq -r '.main.temp')")
        c_code=$(echo "$raw_current" | jq -r '.weather[0].icon')
        c_icon=$(get_icon "$c_code" | cut -d'|' -f1)
        c_hex=$(get_hex "$c_code")
        echo "{\"temp\":\"${c_temp}\",\"icon\":\"${c_icon}\",\"hex\":\"${c_hex}\"}" > "$current_file"
    fi
...
# --- NEW HOURLY MODES FOR TOPBAR ---
    ...
    # Returns "icon|temp" using real-time current_weather.json when available.
    # Falls back to hourly forecast with correct numeric hour comparison (fixes midnight wrap-around bug).
    if [ -f "$current_file" ]; then
        icon=$(jq -r '.icon' "$current_file")
        temp=$(printf "%.1f" "$(jq -r '.temp' "$current_file")")
        echo "$icon|$temp"
    elif [ -f "$json_file" ]; then
        ct_h=$(date +%-H)
        icon=$(jq -r --argjson h "$ct_h" \
            '(.forecast[0].hourly | map(select((.time | split(":")[0] | tonumber) <= $h)) | last) // .forecast[0].hourly[0] | .icon' \
            "$json_file")
        temp=$(jq -r --argjson h "$ct_h" \
