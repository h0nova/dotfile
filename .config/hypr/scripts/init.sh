...
# If the flag exists and the cached wallpaper is still present (same session), just reload colors
if [ -f "$FLAG" ] && [ -f "/tmp/lock_bg.png" ]; then
...
if [ -n "$file" ]; then
    ...
    awww img "$file" --resize fit --transition-type any --transition-pos 0.5,0.5 --transition-fps 144 --transition-duration 1 &
